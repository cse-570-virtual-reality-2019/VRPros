...
class MapWaterbodyCreator : MapInfra
{
    ...
    protected override void OnObjectCreated(List<Vector3> normals, List<Vector2> xys, List<int> indices, MapPaths way, Vector3 origin, List<Vector3> vectors)
    {
        ...
        for (int i = 1; i < length; i++)
        {
            ...
            var crossing = Vector3.Cross(normalisedDifference, Vector3.up) * v;
    ...
    IEnumerator Start()
    {
        ...
        foreach (var way in map.ways.FindAll((w) => { return w.waterBodyExist; }))
        {
            CreateObject(way, waterMat, "WaterBody");
            yield return null;
        }
    }
